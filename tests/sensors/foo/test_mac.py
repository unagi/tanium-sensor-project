...
def test_mac_sanitizes_usernames(tmp_path: Path) -> None:
    base_dir = prepare_sensor_files("foo", "mac", tmp_path)
...
def test_mac_handles_iterdir_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, exception: OSError) -> None:
    base_dir = prepare_sensor_files("foo", "mac", tmp_path)
