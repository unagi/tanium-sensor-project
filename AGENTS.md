...
- Place test collateral under `tests/`. Store fixture trees under `sensors/<sensor>/fixtures/<os>/files/...`.
...
2. Build fixtures first, then add tests under `tests/sensors/<sensor>/test_<os>.py` that call `prepare_sensor_files()`.
